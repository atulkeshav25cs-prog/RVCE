...
export default function AuthorityLogin() {
  ...
  const handleOAuth = async () => {
    ...
    try {
      const result = await signIn?.sso({
        ...
        redirectUrl: "/authority/dashboard",
  ...
  return (
    <section className="relative z-10 flex flex-col items-center justify-center min-h-[90vh] py-32 px-6">
      <div className="bg-white/95 backdrop-blur-md border border-slate-200/50 p-8 sm:p-12 rounded-3xl w-full max-w-md shadow-premium">
        <div className="mb-10 text-center">
          ...
          <h1 className="text-3xl font-bold tracking-tight text-slate-900 mb-2 leading-tight">
            Official Access
          </h1>
          <p className="text-slate-600 font-medium text-sm">
            Secure login for registered dispatch and tactical units.
          </p>
        </div>
        ...
        {!verifying ? (
          <>
            ...
            <div className="flex items-center gap-4 mb-8">
              ...
              <span className="text-slate-400 text-xs font-bold uppercase tracking-widest">Or login via email</span>
              <div className="h-px bg-slate-200 flex-grow"></div>
            </div>
            ...
            <form onSubmit={handleSendCode} className="space-y-6">
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-white border border-slate-300 rounded-lg px-4 py-3 text-slate-900 focus:outline-none focus:border-slate-500 transition-colors" 
              ...
              <button type="submit" disabled={loading} className="w-full bg-slate-900 text-white font-bold uppercase tracking-[0.1em] text-xs py-4 rounded-lg hover:bg-slate-800 transition-all flex items-center justify-center">
                {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : "Request Access Token"}
              </button>
            </form>
          </>
        ) : (
          <form onSubmit={handleVerifyCode} className="space-y-6">
            <div>
              ...
              <p className="text-slate-500 text-xs text-center mb-6">Enter the 6-digit clearance code sent to {emailAddress}</p>
              <input 
                type="text" 
                value={code} 
